    ...
    <div class="min-h-screen bg-copper-50 flex items-center justify-center">
        ...
        <div v-if="showStep1" class="fixed inset-0 flex items-center justify-center bg-black/30 z-30">
            <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full flex flex-col gap-4">
                <h2 class="font-bold text-xl mb-2 text-copper-900">Modifier la note</h2>
        ...
        <div v-if="showStep2" class="fixed inset-0 flex items-center justify-center bg-black/30 z-40">
            <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full flex flex-col gap-4">
                <h2 class="font-bold text-xl mb-2 text-copper-900">Modifier les tags (optionnel)</h2>
...
onMounted(async () => {
  try {
    const res = await fetch(`http://localhost:3001/notes/${route.params.id}`)
    const note = await res.json()
    noteContent.value = note.content
    noteTitle.value = note.title
    noteCategory.value = note.category
    tags.value = note.tags || []
    taskList.value = note.taskList || []
    originalDate = note.date // pour garder la date d'origine
  } catch (e) {
    errorMsg.value = "Erreur lors du chargement de la note."
  }
  loading.value = false
})
...
async function submitEdit() {
  ...
  try {
    await fetch(`http://localhost:3001/notes/${route.params.id}`, {
      method: "PUT",
      ...
      body: JSON.stringify({
        ...
        ...(noteCategory.value === "TaskList" ? { taskList: taskList.value } : { taskList: undefined })
      })
    });
    showStep1.value = false
    showStep2.value = false
    setTimeout(() => router.push("/user-home"), 900)
  } catch (e) {
    errorMsg.value = "Erreur lors de la modification. Réessaie."
  }
}
</script>
